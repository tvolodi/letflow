...
| `db_load.sh` | a load generator that was **written but NEVER EXECUTED** — an unused verification contingency. Produced none of this document's figures. Listed only so its presence is never again mistaken for a record of its use (§2.2). | ISSUE-FIXER (authored; not run) | §2.2 correction note |
...
- On a **demonstrably idle host** (`docker stats` measured `letflow-postgres-1`
  ...
  of 1.38×.**
- On the same host under **observed concurrent sibling-workspace load**, single-process still, n=40:
  **8 of 40 provisions (20%) exceed the entire `claim(0)` budget**, with a maximum of
  **15 373 ms — 3.07× the budget.**

  **"Loaded" here means real, observed, concurrent sibling-workspace activity — not a
  synthetic stressor.** This is stated precisely because **floor 1 (15 373 ms) comes from
  this condition**, so its provenance is load-bearing. At the time samples A and B were
  taken, this host was running **13 BEAM VMs across three Letflow checkouts** — six
  `mix test --partitions 16` and three `mix test --partitions 4` belonging to other
  workspaces, plus this run's own four — with `docker stats` showing `letflow-postgres-1`
  at **197%** and then **125%** CPU at 4.5 GiB RSS, and that container's Postgres
  refusing new connections outright with `FATAL: sorry, too many clients already`.

  That is this project's documented normal operating condition, not an approximation of
  one: `docs/anti-patterns.md`'s "Running `docker compose up` from a secondary worktree
  checkout" documents the several-workspaces-each-with-its-own-Postgres arrangement this
  repo is actually checked out into, and `0009-test-parallel-pool-sizing.md`'s entire
  premise is N concurrent partitions on such a host. Floor 1 is therefore measured under
  the real contention this budget must survive.

  **Correction, recorded rather than silently fixed.** Revision `8ff120c` of this section
  asserted the opposite — that the loaded samples were produced by `scratch/db_load.sh`,
  "four synthetic DDL workers" — and applied a ~10% "the generator is marginally lighter
  (28 tables vs 31)" adjustment on that basis. **`scratch/db_load.sh` was never executed.**
  It was written later than samples A and B (`scratch/claim-latency-sample-B.txt` mtime
  01:12, `scratch/db_load.sh` mtime 01:14), only ever `sh -n` syntax-checked, and exists
  solely as an unused contingency for verification; `select count(*) ... where schema_name
  like 'iss0220load%'` returns 0. The attribution was inferred from the file's presence,
  which is precisely what §0's own standard forbids — and the standard did not catch it
  because §0 covers provenance of *output* artefacts while `db_load.sh` is an *input*.
  §0 now states provenance-of-execution separately from provenance-of-authorship. The
  ~10% adjustment is moot and has been deleted with the claim it corrected.

  Note the direction: the corrected provenance makes floor 1 **stronger**, not weaker.
  A maximum measured under real multi-workspace contention is better evidence for a
  budget that must survive real multi-workspace contention than one measured under a
  stand-in for it.
...
nothing else claiming. The precise claim, stated so the host contention above cannot be
read as smuggling *pool* contention back in: the *host* is contended in samples A and B
by the sibling workspaces' own suites; the **pool and the caller are not**.
`TEST_PARALLEL_N=4` is therefore not a precondition of the failure — it only makes it
fail *often*, by supplying host contention the sibling workspaces supplied incidentally.
It is not what makes the budget wrong.
...
| **loaded host, single process** (sample A) — real sibling-workspace contention, §2.2 | direct probe | 20 | 2112 ms | 3407 ms | **15 373 ms** | 3/20 |
| **loaded host, single process** (sample B) — same observed condition | direct probe | 20 | 1119 ms | 2471 ms | 8711 ms | 5/20 |
...
claim-latency-sample-B.txt   : n=20 min=1119ms max=8711ms  median=2471ms mean=3287ms
                               ...
                               -- "loaded" = 13 concurrent BEAM VMs across three
                                  Letflow checkouts, letflow-postgres-1 at 197%/125%
                                  CPU, "too many clients already"; observed, not
                                  generated. See §2.2.
...
1. **Floor 1's artefact is absent.** Of the nine ISSUE-FIXER files in `scratch/` (§0),
   ...
   reported summary this run could not inspect. That is the real limitation here, and it
   is the only one: an earlier revision compounded it with a second, **false** claim —
   that sample A was taken under a deliberately generated load, making floor 1 "the worst
   legitimate provisioning under a load we chose". That is withdrawn. `scratch/db_load.sh`
   was never executed (§2.2); sample A was measured under **observed** concurrent
   sibling-workspace contention, which is the condition this budget must survive rather
   than a stand-in chosen by this run. The choice of load is therefore *not* an input to
   the answer. If sample A's raw series is ever recovered and its maximum differs, **redo
