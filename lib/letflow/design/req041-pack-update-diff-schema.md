...
| `pack_id` | `:string` | `varchar(255)` | `NOT NULL` | The solution pack's identifying key (equivalent to `process_definitions.name`'s role for process defs — a stable human/system identifier, not the surrogate `id`). No R-Co source available to name the exact identifier shape (§0); `varchar(255)` is this project's default string bound absent a narrower one, matching `process_definitions.name`/`def_id`'s precedent (`req027`/`req035`). |
