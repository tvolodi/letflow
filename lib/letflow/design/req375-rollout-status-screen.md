...
| Completed at | `outcome.completed_at` | Formatted via `formatDateTime` (existing `web/src/i18n/format.ts` helper, same one `DefinitionRollbackPage.tsx` uses) when non-null; em-dash placeholder when `null` (only possible while `status === 'pending'`). **This is keyed off `completed_at`'s own nullness, never off `status`** — REQ-374's `record_outcome_result/2` sets `completed_at` the instant an outcome leaves `pending`, for `failed` exactly as much as for `succeeded` (`migration_rollout_test.exs` asserts a non-nil `completed_at` for every outcome, including the one it deliberately poisons to fail). A `failed` row therefore renders the real attempt timestamp here, not an em-dash — confirmed against a real live run by UAT-RUNNER (`test/uat-reports/uat-2026-09-22-WF02-REQ375-e2e-live-20260922.yaml`, "new_finding_2"), which caught this row's own test fixture/assertion (not this design text, which was already consistent with the real contract) wrongly assuming a `failed` row's `completed_at` would be null. `data-testid="rollout-outcome-completed-{tenant_id}"`. |
