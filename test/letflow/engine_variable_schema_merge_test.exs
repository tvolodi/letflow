defmodule Letflow.EngineVariableSchemaMergeTest do
  ...
  # (engine.ex, merge_output_variables/7's `else` clause) has NO reachable
  # end-to-end trigger from complete_task/3, so no test for it is written. Both
  # members of VariableSchema.error_reason() are structurally unreachable from
  # complete_task/3:
  #
  ...
  #   * :invalid_definition_id -- the COLUMN is `null: false` and typed `uuid`
  ...
  #
  # RESOLVED (GH#310 / ISS-0091): this was originally attributed to
  # InstanceProjection.definition_id being typed nullable in Ecto over a NOT NULL
  # column -- fixed at instance_projection.ex's `@type t`, which now types
  # definition_id as Ecto.UUID.t() (see that module's moduledoc). The downstream
  # consequence -- complete_task/3's public @spec advertising the resulting
  # {:variable_schema_lookup_failed, _} tuple as a possible return -- is also
  # fixed: complete_error() no longer lists it (engine.ex). The lookup guard
  # itself (VariableSchema.validate_definition_id/1) is intentionally still in
  # place as that module's own public-API defence, of the same general class as
  # ISS-0089/GH#306's non-map json_schema branch, and remains covered only as a
  # UNIT test (variable_schema_test.exs's AC7 block asserts
  # {:error, :invalid_definition_id} from fetch_schemas/3 directly) rather than by
  # a DB-seeded test that would certify a path production cannot take.
  # ---------------------------------------------------------------------------------
