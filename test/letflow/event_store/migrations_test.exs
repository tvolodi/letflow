  ...
  describe "§4 guard pattern -- every registered tenant-scoped migration is a real no-op without a prefix" do
    setup do
      ...
      on_exit(fn ->
        ...
        # if its name is one of the six event-store table names AND it was absent from
        # the snapshot taken before this test ran. In the passing case this is a no-op,
        # because a correctly-guarded migration creates nothing in public at all -- which
        # is exactly what this test proves.
        strays = MapSet.difference(public_tables(), tables_before)

        for table <- Map.keys(@event_store_tables), MapSet.member?(strays, table) do
